...
def get_adjacency_matrix(distance_df, sensor_ids, normalized_k=0.1):
    ...
    for row in distance_df.values:
        sensor_from = str(int(row[0]))
        sensor_to = str(int(row[1]))
        if sensor_from not in sensor_id_to_ind or sensor_to not in sensor_id_to_ind:
            raise Exception("Error: edge sensors {} and {} not in sensors list".format(row[0], row[1]))
